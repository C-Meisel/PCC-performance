...
save_fi = None # os.path.join(save_folder_loc,'GP_FI_FC_name.png')
save_gof = None #os.path.join(save_folder_loc,'GP_GOF_FC_name.png')
...
X_train, X_test, y_train, y_test, err_train,err_test = train_test_split(X, y,ppd_err, test_size=1/cv, random_state=random_state)
...
if hyp_tune == True:
    ...
    def objective_nll(trial):
        ...
        nll_cv = calc_nll(y_test_all, y_pred_all, y_std_all, ppd_err)
...
if plot_feature_importance == True:
    ...
    # - Modify the x-axis labels before applying LaTeX formatting, this just cleans up the figure
...
if all_gof == True:
    ...
    if err == False:
        ...
        total_uncertainty = np.sqrt(y_std_all**2 + ppd_err**2)
        coverage = ((y_test_all >= y_pred_all - 1.96 * total_uncertainty) & 
                    (y_test_all <= y_pred_all + 1.96 * total_uncertainty)).mean()
    elif err == True:
        ax5.errorbar(range(len(y_test_all)), y_test_all, yerr=ppd_err, color = '#09396C', fmt='o', label='Actual values')
    ...
    # Boilermaker Gold: #CFB991    Colorado Red: #CC4628
    ax5.set_xlabel('Cell')
    ax5.set_ylabel(r'PPD (W/cm$^2$)')
    ...
    ax5.legend(loc='lower left', bbox_to_anchor=(0.0, 0.95))
    ...
    print(r2_str)
    print(f"Random State: {random_state:0.0f}")
