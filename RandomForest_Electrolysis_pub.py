...
The model can analyze EC data gathered from 4411 cells tested at Mines
...
plot_pdp = True
pdps = 5
plot_specific_pdp = None # 'Dried before co-sinter_Yes' 'Positrode thickness (μm)'
all_gof = True
...
save_fi = None # os.path.join(save_folder_loc,'EC_FI_RFR_figure_name.png')
save_pdp =  None # os.path.join(save_folder_loc,'EC_PDP_RFR_figure_name.png')
save_gof = None #os.path.join(save_folder_loc,'EC_GOF_RFR_figure_name.png')
save_ind_pdp = None # os.path.join(save_folder_loc,'EC_PDP_RFR_indv_figure_name.png')
...
X.rename(columns={'Absolute humidity at co-sinter (g/m^3)': 'Absolute humidity at co-sinter (g/m$^3$)'}, errors='raise',  inplace=True)
...
if plot_feature_importance == True:
    ...
    for label in df['Feature']:  # Use the original labels from the dataframe
        if label.startswith('Negatrode batch_'):
            ...
            new_label = label[:-1]
            new_labels.append(new_label)
        elif label.startswith('Co-sinter batch'):
            new_label = label[:-1]
            new_labels.append(new_label)
        else:
...
if plot_pdp == True:
    ...
    if pdps == 3:
        ...
        for i, feature in enumerate(top_features):  
            ...
            ax.text(-0.05,1.04, letter, fontsize=subfig_label_fontsize,
                        ha='right',va='bottom',weight='bold',transform = ax.transAxes,**ssp_font) 
        
            if i == 0:
                plt.ylabel(r'CD at 1.3 V (A/cm$^2$)') 
            else:
                plt.ylabel('')

            if feature == 'Electrolyte thickness to grain size ratio':
                plt.xlabel('Thickness : Grain size')
            if feature == 'Absolute humidity at co-sinter (g/m^3)':
                plt.xlabel(r'Humidity at co-sinter (g/m$^3$)')
        ...
        # axes = axes[:pdps]  # Limit to the first 5 axes
        ...
        for i, feature in enumerate(top_features):  
            ...
            if feature == 'Absolute humidity at co-sinter (g/m$^3$)':
                ...
                # Extract the x-values (absolute humidity) and y-values (PDP output)
                line = display.lines_[0][0]  # Accessing the Line2D object
                ah_vals = line.get_xdata()  # Absolute humidity values
                y_vals = line.get_ydata()  # PDP output values

                # Convert to relative humidity
                rh_vals = ah_vals * 4.3  # Assuming a simple scaling factor

                # Get the limits of the primary axis
                min_val = rh_vals[0]
                max_val = rh_vals[-1]

                # Set the secondary x-axis limits to be scaled by 4.3
                ax2.set_xlim(min_val, max_val)

                # Enable top spine on the second x-axis
                ax2.spines['top'].set_visible(True)
                
                ax2.set_xlabel(r'Relative Humidity at 25 $^{\circ}$C (%)')
...
if plot_specific_pdp is not None:
    ...
    if save_ind_pdp is not None:
        fmat = save_ind_pdp.split('.', 1)[-1]
        fig.savefig(save_ind_pdp, dpi=300, format=fmat, bbox_inches='tight')
...
if all_gof == True:
    ...
    ax2.set_ylabel("Residuals",labelpad=-5)
